...
public class ShootTwoBallsBridge {
    ...
    public static void run() {
        ...
        double autondrivespeeds = .8;

        // Shooter speed
        double RPM = 2650;

        double delay = 1.0;             //delay so shooters get up to speed
        double conveystop = 2;          //stop the roller let the shooter catch up
        double conveystart = 3.0;       //start the roller to shoot
        double startDrive = 5.5;        //start moving
        double bridge = 8.7;              //hit the bridge
        double backAway = 10.5;          //when to start backing away from the bridge
        double convStop = 12.75;
        double stop = 13.5;             //stop time
